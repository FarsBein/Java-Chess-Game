// This is a clean version (without the testing portions) of the ChessPiece class.

public class ChessPiecesTestingFree {
    ...
    // pawn
    // 1// checks if the piece is white because they face different side of the
    // board and require different calculations
    // 2// to move a pawn foreword from the white side you -1 so endpoint[1](y) + 1
    // will get us back to the starting position startPoint[1]
    // 3// pawns can move in an angle to take a piece but only forward so check if
    // endPoint[0] -1 or +1 or +0 get us back to startPoint
    // 4// black same thing but because they face different side of the board they
    // require the opposite calculations
    ...
    // bishops
    // 1// x1 - x2 = y1 - y2 most be equal since bishops moves side way ( you can
    // move side way only by adding an equal amount to both y and x)
    ...
    // Castle
    // castle can move in the x OR y direction so it will have to maintain one of
    // its values x or y
    ...
    // king
    // could not come up with an optimized algorithm so i just checked for all 8
    // possible distentions
    ...
    // knight
    // since knight has only 8 possible distentions like the king I checked for all
    // of them
